  ...
  try {
    ...
    await conn.query(
      `UPDATE person_table
       ...
       WHERE person_id = ?`,
      [nextFirstName, nextMiddleName || null, nextLastName, normalizedEmail, person_id],
    );
    ...
    if (accountRows.length > 0) {
      const params = [nextLastName, nextMiddleName || null, nextFirstName, normalizedEmail];
      let passwordSql = "";
      ...
      // ✅ When admin sets a new password: force change + disable OTP
      if (hashedPassword) {
        await conn.query(
          `UPDATE user_accounts
