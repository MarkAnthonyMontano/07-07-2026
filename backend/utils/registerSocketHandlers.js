...
${process.env.DB_HOST_LOCAL}:5173/login
  ...
  app.post("/api/forgot-password", async (req, res) => {
    ...
    try {
      ...
      // =========================
      // 1️⃣ STUDENT
      // Match: student_numbering_table.student_number + user_accounts.email
      // via shared person_id
      // =========================
      const [studentNumberRows] = await db3.query(
        `SELECT snt.person_id
      ...
      if (studentNumberRows.length > 0) {
        const studentPersonId = studentNumberRows[0].person_id;

        // Find the student's user_account using that person_id
        const [studentAccountRows] = await db3.query(
          `SELECT id, email
         FROM user_accounts
        ...
        if (studentAccountRows.length === 0) {
          await insertForgotPasswordAuditLog({ outcome: "FAILED" });
          return res.status(404).json({ message: "Student account not found." });
        }

        const accountEmail = String(studentAccountRows[0].email || "").trim().toLowerCase();
        ...
        await db3.query(
          `UPDATE user_accounts SET password = ?, force_password_change = 1 WHERE id = ?`,
          [hashedPassword, studentAccountRows[0].id]
        );

        await sendResetEmail(accountEmail, newPassword, shortTerm);
        await insertForgotPasswordAuditLog({ outcome: "SUCCESS" });
      ...
      // =========================
      // 2️⃣ REGISTRAR
      // Match: user_accounts.employee_id + user_accounts.email, role = 'registrar'
      // =========================
      ...
      if (registrarRows.length > 0) {
        const accountEmail = String(registrarRows[0].email || "").trim().toLowerCase();
        ...
        await db3.query(
          `UPDATE user_accounts SET password = ?, force_password_change = 1 WHERE id = ?`,
          [hashedPassword, registrarRows[0].id]
        );
      ...
      // =========================
      // 3️⃣ FACULTY
      // Match: prof_table.employee_id + prof_table.email, role = 'faculty'
      // =========================
      ...
      if (facultyRows.length > 0) {
        const accountEmail = String(facultyRows[0].email || "").trim().toLowerCase();
        ...
        await db3.query(
          `UPDATE prof_table SET password = ?, force_password_change = 1 WHERE prof_id = ?`,
          [hashedPassword, facultyRows[0].prof_id]
        );
